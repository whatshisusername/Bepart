...
const createEvent = asyncHandler(async (req,res)=>{
    ...
    const {title,description,date,starttime,endtime,startlocation,endlocation,instagram,twitter,whatsapp}=req.body;
    ...
    const createdevent = await Run.findById(event._id)  
    if (!createdevent){
...
console.log("Date and Time in YYYY-MM-DD hh:mm:ss format: " + date_time);
    ...
    console.log(dt)
    const listofevents = await Run.find({date:date_yyyy_mm_dd}).sort({'updatedAt':-1 });
    const events=[]
    for(var i=0; i< listofevents.length; i++) {  
        //display the array elements  
        const student = await User.findById(listofevents[i].owner);
...
console.log("Date and Time in YYYY-MM-DD hh:mm:ss format: " + date_time);
    ...
    const dt =year+"-"+month+"-"+day
    console.log(dt)
    const listofevents = await Run.find({date: {
        $lt: date_yyyy_mm_dd
     }}).sort({'updatedAt':-1 });
    const events=[]
    for(var i=0; i< listofevents.length; i++) {  
...
const getEventById = asyncHandler(async (req, res) => {
    ...
    const event = await Run.findById(eventId)
...
const joinevent = asyncHandler(async (req, res) => {
    ...
    await Run.updateOne({ _id: eventId }, { $push: { participants: req.user._id } })

    const newevent = await Run.findById(eventId)
    // return response
    return res.status(201).json(
