...
function AddRun() {
  ...
  //to type start location
  const[gform,setgform]=useState('')
  //results on type start location
  const [locations1, setlocations1] = useState([])
  const [startlocation,setstartlocation]=useState()

  //to type end location
  const[gform2,setgform2]=useState('')
  //results on type end location
  ...
  const createevent= async(e)=>{
    ...
    await axios.post('/api/v1/runs/create-event', formData)
  ...
  useEffect(() => {
    const fetchLocations = debounce(async () => {
      ...
      try {
        ...
        console.log("api=", response.data);
        setlocations1(response?.data?.results || []);
      } catch (error) {
        console.error(error);
      }
  ...
  useEffect(() => {
    const fetchLocations = debounce(async () => {
      ...
      try {
        const response = await axios(options);
        console.log("api=", response.data);
        setlocations2(response?.data?.results || []);
      } catch (error) {
        console.error(error);
      }
    }, 500); // debounce for 500ms
  ...
  const options = locations1.map((loc) => ({
  ...
  return (
    ...
    <div className="flex justify-between items-center mt-4">
      ...
      <button onClick={()=>{navigate('/today-events-run')}} className="bg-indigo-500 hover:bg-indigo-600 text-white font-semibold py-2 px-6 rounded-lg shadow-lg transition duration-300 ease-in-out transform hover:scale-105">
          ...
          <button onClick={()=>{navigate('/add-event-run')}}className="bg-rose-500 hover:bg-rose-600 text-3xl text-white font-semibold py-2 px-6 rounded-lg shadow-lg transition duration-300 ease-in-out transform hover:scale-105">
            Add Event
          </button>
      </div>
    </div>
    <h1 className="text-4xl font-bold text-gray-900 text-center leading-tight mb-2 pb-4 relative">
      ...
      <div className="heading text-center  text-2xl m-5 text-gray-800 bg-white">New Event-Run</div>
...
<label for="timeInput">Start-Location</label>
...
<select
          ...
          <label for="timeInput">End-Location</label>
<input type="text" className="title bg-gray-100 border border-gray-300 border-b-0 p-2 mb-0 outline-none" placeholder='type location..' id="timeInput" name="timeInput" value={gform2} onChange={(e)=>setgform2(e.target.value)}></input>
