...
var dayNames = ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"];
    ...
    useEffect(()=>{
      axios.get('/api/v1/beachcleanups/upcoming-events')
  ...
  if(events.length===0){
    ...
    <div className="flex justify-between items-center mt-4">
      ...
      <button onClick={()=>{navigate('/today-events')}} className="bg-indigo-500 hover:bg-indigo-600 text-white font-semibold py-2 px-6 rounded-lg shadow-lg transition duration-300 ease-in-out transform hover:scale-105">
          ...
          <button onClick={()=>{navigate('/upcoming-events')}}  className="bg-teal-500 hover:bg-teal-600 text-3xl text-white font-semibold py-2 px-6 rounded-lg shadow-lg transition duration-300 ease-in-out transform hover:scale-105">
            Upcoming
          </button>
    ...
    <h1 className="text-4xl font-bold text-gray-900 text-center leading-tight mb-2 pb-4 relative">
      ...
      <h1 class="mb-4 mt-64 text-3xl text-center font-extrabold text-gray-900 dark:text-white md:text-4xl lg:text-4xl"><span class="text-transparent bg-clip-text bg-gradient-to-r to-emerald-600 from-sky-400">No Upcoming Events</span> </h1>
      </>
    )
  }
